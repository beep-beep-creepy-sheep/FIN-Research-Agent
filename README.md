...
- Web API：FastAPI 提供公司、财务、价格、文档、研究、自选股和任务接口。
- 前端骨架：Next.js 工作台和公司页面。
...
- PDF 表格抽取还不是页码级审计工具。
- 前端是产品骨架，图表和任务中心还需继续完善。
- 数据库表目前由 SQLAlchemy `create_all` 初始化；正式生产化可加入 Alembic 迁移。

## Roadmap

- 公司详情页：财务趋势、利润表、资产负债表、现金流量表、价格图。
- 任务中心：进度、阶段、错误原因、重试。
