...
class PeriodNormalizationService:
    def normalize(self, periods: tuple[FinancialPeriod, ...]) -> PeriodNormalizationResult:
        ...
        cumulative = self._latest_cumulative_by_quarter(periods)
        ...
        for key in sorted(cumulative):
            ...
            previous_cumulative = cumulative.get(key.previous())
            for code, value in current.values.items():
                if code in FLOW_METRICS:
                    quarter_value, metric_fact_ids, metric_urls, formula = self._flow_quarter_value(
                        code, key, current, previous_cumulative
                    )
    ...
    def _latest_cumulative_by_quarter(
        self,
        periods: tuple[FinancialPeriod, ...],
    ) -> dict[QuarterKey, FinancialPeriod]:
        by_key: dict[QuarterKey, FinancialPeriod] = {}
    ...
    def _flow_quarter_value(
        ...
        return (
            ...
            f"{code} Q{key.quarter} = {current.period_end} cumulative - {previous_cumulative.period_end} cumulative",
        )
